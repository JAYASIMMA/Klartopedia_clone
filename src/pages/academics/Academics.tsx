// src/pages/academics/Academics.tsx
import React, { useState, useEffect } from "react";
import Sidebar from "../../components/Sidebar";
import ThemeSettings from "../../components/ThemeSettings";
import { FiMenu, FiBell, FiSettings, FiSun, FiMoon, FiEdit, FiTrash2 } from "react-icons/fi";
import Swal from "sweetalert2";
import { getAcademics, addAcademic, updateAcademic, deleteAcademic } from "../../services/academicService";
import { useTheme } from "../../context/ThemeContext";

interface Academic {
  id: number;
  academic_year: string;
  active: number; // Only 0 or 1
}
...
const Academics: React.FC = () => {
  ...
  const loadAcademics = async () => {
    ...
    try {
      ...
      setAcademics(normalizedData);
      setCurrentPage(1);
    } catch (err: any) {
      console.error("Error loading academics:", err.response?.data || err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editIndex !== null) {
        const item = academics[editIndex];
        await updateAcademic(item.id, { academic_year: inputValue });
        Swal.fire({ icon: "success", title: "Updated!", timer: 1200, showConfirmButton: false });
      } else {
        await addAcademic({ academic_year: inputValue, active: activeStatus });
      ...
      console.error("Error saving academic:", err.response?.data || err.message);
  ...
  const handleDelete = async (index: number) => {
    ...
    const result = await Swal.fire({
      title: "Are you sure?",
      text: `You are about to delete "${item.academic_year}".`,
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        await deleteAcademic(item.id);
        Swal.fire({ icon: "success", title: "Deleted!", timer: 1200, showConfirmButton: false });
        await loadAcademics();
      } catch (err: any) {
        console.error("Error deleting academic:", err.response?.data || err.message);
        ...
        <div className={`flex-1 transition-all duration-300 p-4 ${isSidebarOpen ? "md:ml-[240px]" : "md:ml-[70px]"}`}>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow p-5">
              <h2 className="text-base font-semibold mb-3">{editIndex !== null ? "Edit Academic Year" : "Add Academic Year"}</h2>
              <input
                type="text"
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow p-5 overflow-auto">
              <h2 className="text-base font-semibold mb-3">Academics List</h2>
              {loading ? (
                <p>Loading...</p>
              ) : (
                      ...
                      academics
                        ...
                        .map((item, index) => (
                          <tr key={item.id} className="border-b border-slate-100 dark:border-slate-700">
                            ...
                            <td className="py-2 pr-2">{item.academic_year}</td>
                            ...
                            <td className="py-2 pr-2">
                              <div className="flex items-center gap-3">
                                <FiEdit
                                  ...
                                  onClick={() => {
                                    setInputValue(item.academic_year);
              ...
              {academics.length > pageSize && (
                <div className="mt-3 flex items-center justify-end gap-2">
                  ...
                  {getPageNumbers(Math.ceil(academics.length / pageSize), currentPage).map((p, i) => (
                    typeof p === 'number' ? (
                      <button
                        key={i}
                  ...
                  <button
                    className="px-3 py-1 rounded border bg-white text-slate-700 border-slate-300 hover:bg-slate-100 dark:bg-slate-800 dark:text-slate-100 dark:border-slate-600 dark:hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed"
                    disabled={currentPage === Math.ceil(academics.length / pageSize)}
                    onClick={() => setCurrentPage(p => Math.min(Math.ceil(academics.length / pageSize), p + 1))}
                  >
                    Next
                  </button>
                </div>
